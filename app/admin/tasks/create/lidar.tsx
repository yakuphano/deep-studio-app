...
export default function CreateLidarTaskScreen() {
  ...
  const uploadBytesToStorage = async (
    ...
    const path = `lidar/${user.id}/${Date.now()}_${safe}`;
    ...
    try {
      ...
      if (sourceType === 'local' && selectedFile) {
        ...
        if (urls.length === 1 && isZipDatasetUrl(urls[0])) {
          notify(
            'Uzak ZIP içe aktarma LiDAR görev tipinde henüz desteklenmiyor. Yerel .zip yükleyin veya tek tek görüntü URL’leri kullanın.'
          );
          setIsCreating(false);
          return;
        }
        ...
        logImportFailures(payload.results, 'CreateLidarTask');
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <Text style={styles.title}>Create LiDAR Annotation Task</Text>
        <Text style={styles.hint}>
          LiDAR BEV / yansıtılmış görüntü görevleri yerel dosya veya uzak URL ile oluşturulur; dosyalar
          task-assets/lidar/… altına yüklenir.
        </Text>
