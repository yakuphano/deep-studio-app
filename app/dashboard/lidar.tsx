...
export default function LidarTasksScreen() {
  ...
  const fetchTasks = useCallback(
    async (showLoading = true) => {
      ...
      try {
        ...
        if (error) {
          console.error('[lidar-tasks] Fetch error:', error);
          return;
        }
    ...
    <View style={styles.container}>
      <View style={styles.breadcrumbRow}>
        <TouchableOpacity style={styles.backBtn} onPress={handleBack}>
          <Ionicons name="arrow-back" size={16} color="#fdba74" />
          <Text style={styles.backText}>Back</Text>
        </TouchableOpacity>
        <Text style={styles.breadcrumbText}>
          {`${t('nav.dashboard')} > ${t('nav.breadcrumbLidar')}`}
        </Text>
      </View>

      <View style={styles.pageHeader}>
        <Text style={styles.pageTitle}>{t('tasks.pageTitleLidar')}</Text>
      </View>
      ...
      <View style={styles.body}>
        {loading ? (
          ...
          <View style={styles.emptyWrap}>
            <Ionicons name="scan-outline" size={64} color={ACCENT} />
            <Text style={styles.emptyTitle}>No LiDAR tasks</Text>
          ...
          <View style={styles.gridContainer}>
            <FlatList
              ...
              renderItem={({ item }) => (
                <TaskListCard
                  title={`${t('tasks.taskListHeadingLidar')} — ${item.title}`}
                  status={item.status}
                  price={item.price}
                  accent={ACCENT}
                  icon="scan"
                  ...
                  onPress={() => router.push(`/dashboard/lidar/${item.id}`)}
                />
              )}
...
const styles = StyleSheet.create({
  ...
  backText: { color: '#fdba74', fontSize: 14, fontWeight: '600' },
